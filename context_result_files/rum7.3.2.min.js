...
rumjs.geoloc={init:function(){},getCountry:function(){var a=rumjs.geoloc.sanitizeCountry(rumjs.config.getCookie(rumjs.config.geoloc.cookie.name));return rumjs.geoloc.validateCountry(a)?(rumjs.config.logInfo("Geoloc: Retrieved country from cookie:",a),a):null},getAsyncCountry:function(){if(rumjs.config.geoloc.enabled){var a=rumjs.geoloc.getCountry();null!==a?(rumjs.geoloc.sendGeoLoadedEvent(a),rumjs.config.logInfo("Geoloc: Won't make external request for country. Retrieved country locally.:",a)):a===
rumjs.config.geoloc.cookie.retryValue?(rumjs.geoloc.sendGeoLoadedEvent(a),rumjs.config.logInfo("Geoloc: No country retrieved locally. But we won't retry as we have default value:",a)):(rumjs.config.logInfo("Geoloc: Preparing request to get country..."),rumjs.geoloc.geoJS.getAsyncCountry(),rumjs.config.logInfo("Geoloc: Request to get country sent."))}else rumjs.config.logInfo("Geoloc: getCountry - this feature is not enabled in config.")},sanitizeCountry:function(a){return a?a.trim().toLowerCase():
a},validateCountry:function(a){return a?2===a.length:!1},saveCountry:function(a){if(!rumjs.config.geoloc.enabled)return rumjs.config.logInfo("Geoloc: getCountry - this feature is not enabled in config."),null;a=rumjs.geoloc.sanitizeCountry(a);rumjs.geoloc.validateCountry(a)?(rumjs.config.logInfo("Geoloc: Storing country in cookie. Country is valid:",a),rumjs.geoloc.sendGeoLoadedEvent(a),rumjs.config.setCookie(rumjs.config.geoloc.cookie.name,a,{path:"/",expires:rumjs.config.geoloc.cookie.successExpiration,
domain:"."+location.hostname.split(".").reverse().splice(0,2).reverse().join(".")})):(rumjs.config.logInfo("Geoloc: Country is not valid:",a),rumjs.geoloc.saveErrorNoCountry())},saveErrorNoCountry:function(){if(!rumjs.config.geoloc.enabled)return rumjs.config.logInfo("Geoloc: getCountry - this feature is not enabled in config."),null;rumjs.config.logInfo("Geoloc: setting cookie to not retry in next",rumjs.config.geoloc.cookie.retryExpiration,"days");rumjs.config.setCookie(rumjs.config.geoloc.cookie.name,
rumjs.config.geoloc.cookie.retryValue,{path:"/",expires:rumjs.config.geoloc.cookie.retryExpiration,domain:"."+location.hostname.split(".").reverse().splice(0,2).reverse().join(".")})},sendGeoLoadedEvent:function(a){rumjs.gtmque({event:"geoLoaded",country:a})},geoJS:{endpointURL:"https://get.geojs.io/v1/ip/country",trackStatus:function(a,b){rumjs.gtmque({event:"geojsRequest_"+a,event_value:b})},getAsyncCountry:function(){var a=new XMLHttpRequest;a.onreadystatechange=function(){try{4===this.readyState&&
(rumjs.config.logInfo("Geoloc: Response received"),200===this.status?(rumjs.geoloc.saveCountry(this.responseText),rumjs.geoloc.geoJS.trackStatus("success","")):(rumjs.geoloc.saveErrorNoCountry(),rumjs.geoloc.geoJS.trackStatus("failed",this.status)))}catch(b){rumjs.config.logError("Geoloc: error while processing the repsonse."),rumjs.geoloc.saveErrorNoCountry(),rumjs.geoloc.geoJS.trackStatus("failed","exception_readyState")}};a.ontimeout=function(){rumjs.geoloc.saveErrorNoCountry();rumjs.geoloc.geoJS.trackStatus("failed",
"timeout")};a.open("GET",this.endpointURL);a.timeout=rumjs.config.geoloc.timeout;a.send();rumjs.config.logInfo("Geoloc: GeoJS - Sending request to get country...")}}};
